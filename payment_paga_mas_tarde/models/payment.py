    ...
    def pmt_form_generate_values(self, values):
        ...
        pmt_tx_values.update({
            "configuration": {
                ...
                "urls": {
                    "ok": '%s' % urljoin(base_url, PmtController._ok_url),
                    "ko": '%s' % urljoin(base_url, PmtController._ko_url),
                    "cancel": '%s' % urljoin(base_url, PmtController._cancel_url),
                    "authorized_notification_callback": '%s' % urljoin(
                        base_url, PmtController._notification_url),
                    "rejected_notification_callback": '%s' % urljoin(
                        base_url, PmtController._notification_url)
                }
            },
            "metadata": {
