...
class Crumb(models.Model):
    _name = 'breadcrumbs.crumb'
    _description = 'Website Breadcrumb'

    name = fields.Text(translate=True)
    url = fields.Text()
    active = fields.Boolean()
    website_published = fields.Boolean(
        string=_('Website Published'),
        default=True,
...
def _generate_one(name, url, active):
    ...
    """
    crumb = request.env['breadcrumbs.crumb'].sudo()
    result = crumb
    if url:
        domain_crumb = [('url', '=', url)]
        domain_crumb = expression.AND(
            [
                domain_crumb,
...
class BreadCrumbs(models.Model):
    ...
    def generate_breadcrumbs(self, main_object, website):
        breadcrumbs = self.env['breadcrumbs.crumb'].sudo()
        ...
        if main_object._name == 'product.template':
            ...
            if product.public_categ_ids:
                ...
                if parent_cat:
                    ...
                    for res in parent_res:
                        ...
                        cat_url = '/category/%s' % cat.slug \
                            if cat.slug else '/shop/category/%s' % slug(cat)
            ...
            tag = main_object
            tag_url = '/tag/%s' % tag.slug \
                if tag.slug else '/shop/tag/%s' % slug(tag)
            ...
            if page.parent_id:
                ...
                parent_res.reverse()
                for res in parent_res:
                    parent = self.env['website.page'].sudo().search([
                        ('id', '=', res)]
                    )
                    breadcrumbs += _generate_one(
                        parent.name or parent.view_id.name, parent.url, False
                    )
            breadcrumbs += _generate_one(
                page.name or page.view_id.name, page.url, True
            )
            ...
            # If there is no page with current view
            # search page with current url
            if not page:
            ...
            if page:
                if page.parent_id:
                    ...
                    for res in parent_res:
                        ...
                        breadcrumbs += _generate_one(
                            parent.name or parent.view_id.name,
                            parent.url,
                            False
                        )
                breadcrumbs += _generate_one(
            ...
            breadcrumbs += _generate_one(
                blog.name, '/blog/%s' % slug(blog), False
            )
            # Add post crumb
            ...
            breadcrumbs += _generate_one(
                channel.name, '/slides/%s' % slug(channel), False
            )
            # Add slide crumb
            ...
            if main_object._description:
                name = _('%s / %s') % (
                    main_object._description, main_object.name
                )
            else:
